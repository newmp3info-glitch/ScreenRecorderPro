...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Spinner>(R.id.resolution).adapter =
            ArrayAdapter(
                this,
        ...
        findViewById<Spinner>(R.id.fps).adapter =
            ArrayAdapter(
                this,
                android.R.layout.simple_spinner_dropdown_item,
                listOf(
    ...
    @Deprecated("Activity result API kept for project compatibility")
    override fun onActivityResult(
        ...
        if (
            requestCode == projectionRequest &&
            resultCode == Activity.RESULT_OK &&
            data != null
        ) {

            val resolution =
                findViewById<Spinner>(R.id.resolution)
                    .selectedItem
                    .toString()

            val fps =
                findViewById<Spinner>(R.id.fps)
                    .selectedItem
                    .toString()
                    .substringBefore(" ")
                    .toIntOrNull() ?: 30
            ...
            val intent =
                Intent(this, RecordingService::class.java).apply {
                    ...
                    putExtra(
                        RecordingService.RESULT_CODE,
                        resultCode
                    )

                    putExtra(
                        RecordingService.PROJECTION_DATA,
                        data
                    )

                    putExtra(
                        RecordingService.RESOLUTION,
                        resolution
                    )

                    putExtra(
                        RecordingService.FPS,
                        fps
                    )

                    putExtra(
                        RecordingService.MIC,
                        mic.isChecked
                    )

                    putExtra(
                        RecordingService.OVERLAY,
                        overlay.isChecked
                    )
                }
        ...
        else if (requestCode == projectionRequest) {

            status.text = "Screen recording permission cancelled."
        }
    }
